// ============================================================
// WIDGET OS SERIAL CONFIG & FACE SUPPORT - Add this to your INO
// ============================================================
// Copy and paste this code into your S3_MiniOS.ino file
...
void sendDeviceStatus() {
  ...
  USBSerial.println("BOARD:WOS-180A");
  USBSerial.println("SCREEN:368x448");
  USBSerial.println("DISPLAY:SH8601");
...
void saveWifiConfigToSD(String config) {
  ...
  if (numWifiNetworks > 0) {
    ...
    WiFi.disconnect();
    delay(500);
    // WiFi will auto-reconnect on next check
  }
}
...
void listInstalledFaces() {
  ...
  // List custom faces from SD card
  if (sdCardInitialized) {
    // Scan custom faces
...
void saveFaceToSD(String faceId, String faceJson) {
  ...
  // Sanitize face ID (remove special characters)
  faceId.replace("/", "");
  faceId.replace("\\", "");
  faceId.replace("..", "");
  ...
  // Rescan faces to update the list
  scanSDFaces();
...
void setActiveFace(String faceId) {
  ...
  // If in face selection mode, exit it
  if (inFaceSelectionMode) {
    exitFaceSelectionMode();
  }
  
  // Trigger UI refresh to show new face
  // (You may need to call your face rendering function here)
...
void checkFaceHoldGesture(bool touchActive, int16_t touchX, int16_t touchY) {
  // Only detect on main watch face screen (category 0 = clock)
  ...
  if (touchActive) {
    if (!faceHoldActive) {
      ...
      // Trigger face selection after full hold duration
...
void showHoldProgress(float progress) {
  // Create progress arc if it doesn't exist
  if (holdProgressArc == nullptr) {
    holdProgressArc = lv_arc_create(lv_scr_act());
    lv_obj_set_size(holdProgressArc, 120, 120);
    ...
    lv_obj_set_style_arc_width(holdProgressArc, 8, LV_PART_MAIN);
    lv_obj_set_style_arc_width(holdProgressArc, 8, LV_PART_INDICATOR);
...
void enterFaceSelectionMode() {
  ...
  lv_obj_set_style_text_font(faceSelectionLabel, &lv_font_montserrat_20, 0);
...
void exitFaceSelectionMode() {
  ...
  // Return to main clock screen
  // (Call your main screen loading function here)
}

void updateFaceSelectionLabel() {
  if (faceSelectionLabel == nullptr) return;
  
  // Get face name at current index
...
int getTotalFaceCount() {
  return 4 + numSDFaces; // 4 built-in + custom
}
...
// ============================================
// ADD TO YOUR TOUCH HANDLER IN loop()
// After reading touch coordinates
// ============================================

/*
// Example integration with your existing touch handling:
...
if (FT3168->IIC_Interrupt_Flag) {
  ...
  // Get touch data
  uint8_t touchCount = FT3168->IIC_Read_Device_Value(FT3168->Arduino_IIC_Touch::Value_Information::FINGERSNUM);
  ...
  if (touchCount > 0) {
    ...
    // Check for face hold gesture
    checkFaceHoldGesture(true, touchX, touchY);
    
    // Handle face selection mode
    if (inFaceSelectionMode) {
      // Detect swipe direction or tap
      // (You'll need to implement swipe detection based on touch start/end positions)
    }
  } else {
    // No touch - release hold
    checkFaceHoldGesture(false, 0, 0);
  }
}
*/

// ============================================
// CALL THESE IN YOUR EXISTING FUNCTIONS:
// ============================================
